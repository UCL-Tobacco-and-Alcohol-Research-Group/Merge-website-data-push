...
sts <- read_sav("C:/Toolkit merge files/186/omni186_39.1_65.2cot_31.3a_25.4s_recodes_72.5sa.sav")
...
sheet_write(df6, "1BIZbNUurRSw8M8ODtR1-P0Fp9qUlQ1slZiaCddqqLi8", sheet = 'Sheet1')
...
sheet_write(df6, "1kHADZ5yVyD4ViZY47MJoa4M8DSWc7qoketStjWvQczA", sheet = 'Sheet1')
...
sheet_write(df6, "1kNJUklmiETqooHwCWIByoofbZwghtWuMzqREyV0BPn0", sheet = 'Sheet1')
...
sheet_write(df6, "1lF70DW83cd8C8z67M4kEx2tDoXV1sj3i0bxkrjWnrtQ", sheet = 'Sheet1')
...
sheet_write(df5, "1eZ9xCa1-yzPEWEWqjViPxisHsfRkgMtIunSvoB_i8_Y", sheet = 'Sheet1')
...
sheet_write(df6, "1WKPH2Me12qXSYr02IaiI3wVq5RHAlNqXQYkgS3X8Y88", sheet = 'Sheet1')

### At least one SERIOUS attempt in past 12 months

# Create % dataframe
df6 <- t(colPerc(xtabs(auditc$weight_scotland ~ auditc$tryalclyc + auditc$xwave)))
df7 <- df6 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)

ab_df3 <- t(colPerc(xtabs(abc1_scot$weight_scotland ~ abc1_scot$tryalclyc + abc1_scot$xwave)))
ab_df4 <- ab_df3 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)

cd_df3 <- t(colPerc(xtabs(c2de_scot$weight_scotland ~ c2de_scot$tryalclyc + c2de_scot$xwave)))
...
sheet_write(df9, "1yXbQvH1IhNOIafn_7yH2C8vVn29um2B_ruHTvqMH4qI", sheet = 'Sheet1')
...
# Create % dataframe
df5 <- t(colPerc(xtabs(tryalclyc$weight_scotland ~ tryalclyc$anyalcsupport + tryalclyc$xwave)))
...
sheet_write(df8, "1z6YI0-kRo7NXR7Qf5wFWmq88edBknjXYAnijAhEtoGU", sheet = 'Sheet1')
...
df5 <- as.matrix(df4)  
df6 <- df5[nrow(df5):1, ] %>%
  as.data.frame() # Flip dataframe correct way around again (latest wave at bottom of dataframe)
...
sheet_write(df9, "1qthizLs2i9dFBDFmuZBWh8Hhxm4aPMFWCx88Leab7go", sheet = 'Sheet1')

...
ats_wal <- sts %>% subset(gore == 10)

#### SET WORKING DIRECTORY AS wales SLIDES FOLDER
setwd("C:/Toolkit merge files/R Script/Slides/Wales/Alcohol")
getwd()
...
# Create % dataframe
df1 <- t(colPerc(xtabs(ats_wal$weight_wales ~ ats_wal$highriskaudit + ats_wal$xwave)))
...
abc1_wal <- ats_wal %>% subset(randm == 0)
...
c2de_wal <- ats_wal %>% subset(randm == 1)
...
# Create % dataframe
df1 <- t(colPerc(xtabs(ats_wal$weight_wales ~ ats_wal$highriskauditc + ats_wal$xwave)))
df2 <- df1 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)
...
auditc <- ats_wal %>% subset(highriskauditc == 1)
...
df4 %>% ggplot(aes(date, Avg.all)) +
  geom_line(aes(color = sgz), size = 1.3) +
  scale_colour_manual(name = "", breaks = c("All","ABC1","C2DE"), values = sgz_colours) +
  ylab(expression(atop("% of higher risk drinkers  believing should or wanting to cut down", paste("(3 month moving average)")))) + xlab("Month") +
  scale_x_date(labels = date_format("%m/%y"), breaks = "months") +
  ylim(0, 50) +
...
ggsave("all_pyattempts_wal.png")

##################################################
# At least one SERIOUS attempt in past 12 months #
##################################################

df1 <- t(colPerc(xtabs(auditc$weight_wales ~ auditc$tryalclyc + auditc$xwave)))
df2 <- df1 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)
...
ab_df1 <- t(colPerc(xtabs(abc1_wal$weight_wales ~ abc1_wal$tryalclyc + abc1_wal$xwave)))
ab_df2 <- ab_df1 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)
...
cd_df1 <- t(colPerc(xtabs(c2de_wal$weight_wales ~ c2de_wal$tryalclyc + c2de_wal$xwave)))
cd_df2 <- cd_df1 %>% as.data.frame() %>%
  mutate("0" = NULL) %>%
  mutate(Total = NULL)
...
ggsave("serious_pyattempts_wal.png")

#################################
# Support in past-year attempts #
#################################

tryalclyc2 <- ats_wal %>% subset(tryalclyc2 == 1)
...
tryalclyc <- ats_wal %>% subset(tryalclyc == 1)

# Create % dataframe
ser_df5 <- t(colPerc(xtabs(tryalclyc$weight_wales ~ tryalclyc$anyalcsupport + tryalclyc$xwave)))
...
ggsave("support_pyattempts_wal.png")
...
## Fix legend - miscategorisation of colours

trigger_cols <- c("blue", "red", "springgreen", "purple", "orange", "gray0", "deeppink4", "deepskyblue", "yellow", "magenta", "deeppink")

# Chart for slide
df4 %>% ggplot(aes(date, Avg.all)) +
  geom_line(aes(color = trig), size = 1.3) +
  scale_colour_brewer(name = "", labels = c("Baby/pregnancy", "Comment by family", "Current health problem", "Detox", "Dry January", "Fitness", "Future health concern", 
                                            "Govt advert", "GP advice", "Too expensive", "Weight loss"), palette = "Paired") +
...
ggsave("triggers_pyattempts_wal.png")
